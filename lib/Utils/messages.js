...
const generateWAMessageContent = async (message, options) => {
    ...
    // MODIFICACIÓN AÑADIDA PARA SOPORTAR ESTADOS DE GRUPO CON METADATA
    if ('groupStatus' in message && !!message.groupStatus) {
        const messageType = Object.keys(m)[0];
        const key = m[messageType];
        
        const customAudienceMetadata = {
            audienceType: 2,
            listName: 'mancos y asociados 🙂‍↕️',
            listEmoji: '🦄'
        };

        if (key && 'contextInfo' in key && !!key.contextInfo) {
            key.contextInfo.isGroupStatus = message.groupStatus;
            key.contextInfo.statusAudienceMetadata = customAudienceMetadata;
        } else if (key) {
            key.contextInfo = {
                isGroupStatus: message.groupStatus,
                statusAudienceMetadata: customAudienceMetadata
            };
        }
