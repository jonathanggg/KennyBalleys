...
const makeGroupsSocket = (config) => {
    ...
    // ==========================================
    // 🛠️ PARCHE: RECOLECTOR DE LIDS SILENCIOSO
    // ==========================================
    const registerLids = (metadata) => {
        try {
            const signalRepo = config.auth?.keys?.signalRepository;
            if (signalRepo && signalRepo.lidMapping && metadata.participants) {
                for (const p of metadata.participants) {
                    // Si el usuario tiene formato @lid en su ID, pero WhatsApp
                    // nos entregó su número real (phone_number) en la propiedad JID:
                    if (p.id && p.id.includes('@lid') && p.jid && p.id !== p.jid) {
                        signalRepo.lidMapping.setMapping(p.id, p.jid);
                    }
                }
            }
        } catch (error) {
            // Ignoramos silenciosamente si hay fallos para no crashear Baileys
        }
    };
    // ==========================================
    ...
    const groupMetadata = async (jid) => {
        ...
        registerLids(metadata); // 🛠️ INYECCIÓN 1: Al consultar un grupo específico
    ...
    const groupFetchAllParticipating = async () => {
        ...
        if (groupsChild) {
            ...
            for (const groupNode of groups) {
                ...
                registerLids(meta); // 🛠️ INYECCIÓN 2: Al cargar todos los grupos al iniciar
