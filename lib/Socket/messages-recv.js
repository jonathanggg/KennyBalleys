...
const shortcake_1 = require("../Utils/shortcake.js");
...
const makeMessagesRecvSocket = (config) => {
    ...
    // ==========================================
    // 🛠️ PARCHE: RECOLECTOR DE LIDS EN EVENTOS ENTRANTES
    // ==========================================
    const extractAndSaveLid = (attrs) => {
        try {
            const signalRepo = config.auth?.keys?.signalRepository;
            if (signalRepo && signalRepo.lidMapping) {
                // Si el remitente es LID y el nodo trae el número real en sender_pn
                if (attrs.from && attrs.from.includes('@lid') && attrs.sender_pn) {
                    signalRepo.lidMapping.setMapping(attrs.from, attrs.sender_pn);
                }
                // Si el participante (grupo) es LID y el nodo trae su número en participant_pn
                if (attrs.participant && attrs.participant.includes('@lid') && attrs.participant_pn) {
                    signalRepo.lidMapping.setMapping(attrs.participant, attrs.participant_pn);
                }
                // Variante secundaria
                if (attrs.participant && attrs.participant.includes('@lid') && attrs.sender_pn) {
                    signalRepo.lidMapping.setMapping(attrs.participant, attrs.sender_pn);
                }
            }
        } catch (error) {
            // Ignorado para no interrumpir el flujo
        }
    };
    // ==========================================

    const shortcakeFlow = config.signPasskeyAssertion
        ? (0, shortcake_1.makeShortcakeFlow)({
            logger,
            query,
            signAssertion: config.signPasskeyAssertion,
            getCreds: () => authState.creds,
            updateCreds: patch => ev.emit('creds.update', patch),
            deviceType: config.browser && config.browser[1] ? config.browser[1].toUpperCase() : 'CHROME',
    ...
    if (shortcakeFlow) {
        ev.on('connection.update', (update) => {
            if (update.connection === 'close' || update.connection === 'lost') {
                shortcakeFlow.clearSession();
            }
        });
    }

    const shortcakeMutex = (0, make_mutex_1.makeMutex)();
    ...
    const handleReceipt = async (node) => {
        extractAndSaveLid(node.attrs); // 🛠️ INYECCIÓN 3
        var _a, _b;
    ...
    const handleNotification = async (node) => {
        extractAndSaveLid(node.attrs); // 🛠️ INYECCIÓN 2
    ...
    const handleMessage = async (node) => {
        extractAndSaveLid(node.attrs); // 🛠️ INYECCIÓN 1
        var _a, _b, _c;
